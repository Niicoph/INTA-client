...
const cards = [
  {
    ...
    description:
      'Estime y compare los costos operativos de distintos conjuntos de maquinaria según las labores de campo.',
    path: '/maquinaria',
  },
  {
    src: '/images/Sanidad.png',
    alt: 'Sanidad Icon',
    color: {
      bg: 'bg-green-400',
      text: 'text-green-400',
    ...
    description:
      'Evalúe planes de control fitosanitario, analice costos por producto y estrategia, y optimice sus decisiones en función del rendimiento y del presupuesto.',
    path: '/sanitizacion',
  },
  {
    src: '/images/Fertilizacion.png',
    ...
    description:
      'Calcule el gasto total de fertilizantes por hectárea y compare estrategias para optimizar la eficiencia técnica y económica.',
    path: '/fertilizacion',
  },
];
...
const features = [
  ...
  },
  {
    ...
    description:
      'Reportes detallados con visualizaciones avanzadas para la toma de decisiones estratégicas.',
  },
  {
    title: 'Soporte Económico para la Gestión',
    description:
      'Información económica clave para mejorar la planificación y gestión estratégica del negocio agropecuario.',
  },
];

export default function Home() {
  return (
    <main className="flex justify-center bg-[#F3F7FD]">
        ...
        <section className="flex flex-1 flex-col md:flex-0">
          ...
          <div className="w-full relative ">
            <div className="h-[340px]  w-full max-w-sm smm:h-[400px] md:h-[500px] 2xl:h-[600px] xl:max-w-3xl">
              <picture>
                <source media="(min-width: 1536px)" srcSet="/images/bg-2xl2.png" />
                <source media="(min-width: 1024px)" srcSet="/images/bg-xl.png" />
                <source media="(min-width: 640px)" srcSet="/images/bg-md.png" />
                <img
                  src="/images/bg.png"
                  alt="Fondo home"
              ...
              <div className="absolute flex flex-col items-end mr-3 gap-4 h-full md:hidden ">
                {features.map((feat, index) => (
                  <div
                    key={index}
                    className="w-2/3 bg-white/50 backdrop-blur-md shadow-lg rounded-xl p-4 text-[#4A4A4A]"
                  >
        ...
        <section className="hidden md:flex justify-center m-5">
          <div className="w-full grid grid-cols-3 lg:w-5/6 2xl:w-4/6 gap-5">
            {features.map((feat, index) => (
              <div
                key={index}
                className="bg-white/50 backdrop-blur-md shadow-lg rounded-xl p-4 text-[#4A4A4A]"
              >
                <h3 className="gilroy-bold text-black text-2xl">{feat.title}</h3>
                <p className="m-2 gilroy-regular text-[#4A4A4A] text-base">{feat.description}</p>
              </div>
            ))}
          </div>
        </section>
