...
export default function FormTratamiento({ planControl, index }: FormTratamientoProps) {
  ...
  return (
    <div className="w-full h-full p-4 gap-4 shrink-0 overflow-scroll flex flex-col border border-border rounded-md">
      <FormField
        control={planControl}
                          ...
                          <SelectContent>
                            {productos?.map((producto: Producto) => (
                              <SelectItem key={producto.id_sanitizante} value={producto.id_sanitizante}>
                                {producto.nombre} ({producto.tipo}) {producto.volumen_envase}{producto.unidad}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
