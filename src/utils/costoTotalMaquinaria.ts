...
export const calcularCostoTotalMaquinaria = ( data: MaquinariaFormData[] ): CostoEconomico[] => {
  return data.map((conjunto, index) => {
    ...
    const costo_combustible =
      Number((conjunto.potencia_CV * conjunto.consumo_litros_hora_CV * conjunto.cotizacion_gasoil_litro).toFixed(2));
    ...
    const amortizacion_t =
      Number((((conjunto.precio_usd_t - valor_residual_t) / conjunto.horas_utiles_t) * conjunto.cotizacion_usd).toFixed(2));
